...
After solving for the navigation solution via WLS, the residual vector is computed for each epoch to quantify the measurement error:$$r = y - A \hat{p}$$
...
where \(\sigma_r^2\) is the estimated variance of the residuals, normalized by the degrees of freedom \((n - 4)\) where \(n\) is the number of satellites.
...
- For a false alarm probability \(P_{fa} = 0.02\), the critical value is computed as:

$$
\chi^2_{critical} = \chi^2_{\alpha, \text{dof}}
$$

where \(\alpha = 0.02\) and degrees of freedom \(\text{dof} = n-4\).

- For a missed detection probability \(P_{md} = 10^{-7}\), the threshold for protection level is computed with:
...
f. Fault Exclusion (Not implemented)
...
3. Protection Level (PL) Computation
...
4. Visualization and Stanford Chart Analysis
